      ...
      <div className="relative bg-card border rounded-2xl shadow-2xl w-full max-w-md animate-fade-in">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
          <div>
            ...
            <input {...register('title')} placeholder="תזכורת..." className="w-full px-3 py-2 rounded-xl border bg-background text-sm focus:outline-none transition-all" />
            {errors.title && <p className="text-destructive text-xs mt-1">{errors.title.message}</p>}
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground block mb-1">תיאור</label>
            <textarea {...register('description')} rows={2} placeholder="פרטים נוספים..." className="w-full px-3 py-2 rounded-xl border bg-background text-sm focus:outline-none transition-all resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-muted-foreground block mb-1">סוג</label>
              <select {...register('type')} className="w-full px-3 py-2 rounded-xl border bg-background text-sm focus:outline-none transition-all">
            ...
            <div>
              ...
              <select {...register('clientId')} className="w-full px-3 py-2 rounded-xl border bg-background text-sm focus:outline-none transition-all">
          ...
          <div>
            ...
            <input {...register('remindAt')} type="datetime-local" className="w-full px-3 py-2 rounded-xl border bg-background text-sm focus:outline-none transition-all" />
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={loading} className="flex-1 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 disabled:opacity-60 flex items-center justify-center gap-2">
